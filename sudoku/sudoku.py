...
class Sudoku(SearchProblem):
    ...
    def is_goal(self, board):
        #ここを埋める
        if Board(board.data).verify() == True and Board(board.data).filled() == True:
            return True
        else:
            return False
    ...
    def next_states(self, board):
        ...
        while i <= 9:
            for x in range(0,len(board.data)):
                for y in range(0,len(board.data[x])):
                    potencial[x].append(Board(board.data).get_allowed_digits(x,y))
                    if board.data[x][y] == 0 and len(potencial[x][y]) == i:
                        while potencial[x][y]:
                            allowed_digits = Board(board.data).move(x,y,potencial[x][y].pop(0))
                            check1.append(allowed_digits)
                        if check1 is None:    
                            return []
                        else:
                            return check1
